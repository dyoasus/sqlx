...
import (
	...
	"github.com/dyoasus/sqlx/reflectx"
	_ "github.com/lib/pq"
...
func loadDefaultFixtureContext(ctx context.Context, db *DB, t *testing.T) {
	...
	tx.MustExecContext(ctx, tx.Rebind("INSERT INTO person (first_name, last_name, email) VALUES (?, ?, ?)"), "Jason", "Moiron", "dyoasus@dyoasus.net")
...
func TestUsageContext(t *testing.T) {
	RunWithSchemaContext(context.Background(), defaultSchema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		if jason.Email != "dyoasus@dyoasus.net" {
			t.Errorf("Expecting Email of dyoasus@dyoasus.net, got %s", jason.Email)
		}
